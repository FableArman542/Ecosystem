...
public class FlockOverGrid extends Flock {
	public FlockOverGrid(PApplet p, int nboids, float radius, float hue, boolean ps) {
		super(p, nboids, radius, hue, ps);
	}
	...
	protected void createFlock(PApplet p, int nboids) {
		...
		for (int i = 0; i < nboids; i++) {
			...
			Boid b = new BoidOverGrid(p, new PVector(p.random(p.width), p.random(p.height)), 1, color, radius);
			boids.add(b);
		}
	...
	public void applyBehaviour(Boid b, Minefield t) {
		...
		PVector f = ((BoidOverGrid)b).avoidTrees(t);
		b.applyForce(fa.add(f));
	}
}
